...
def _parse_tool_call(tc: dict) -> tuple[str, str, str] | None:
    """Validate and extract (name, tool_call_id, arguments) from a tool call dict.

    Returns None if the payload is malformed, after logging a warning.
    """
    try:
        func = tc.get("function") or {}
        name = func.get("name")
        if not name:
            LOGGER.warning("Skipping malformed tool call: missing 'name'.")
            return None
        tc_id = tc.get("id")
        if not tc_id:
            LOGGER.warning("Skipping malformed tool call: missing 'id'.")
            return None
        args = func.get("arguments", "{}")
        return name, tc_id, args
    except KeyError as e:
        LOGGER.warning("Skipping malformed tool call due to missing key: %s", e)
        return None
