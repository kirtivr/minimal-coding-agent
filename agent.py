...
def _process_single_tool_call(tc: dict) -> dict | None:
    """Process a single tool call defensively.

    Validates required keys, dispatches the tool, and returns the result
    dictionary. Returns None and logs on malformed input or dispatch failure.
    """
    func = tc.get("function")
    if not isinstance(func, dict):
        LOGGER.warning("Malformed tool call: missing or invalid 'function' field.")
        return None
    ...
    if not tool_call_id:
        LOGGER.warning("Malformed tool call: missing 'id'.")
        return None
